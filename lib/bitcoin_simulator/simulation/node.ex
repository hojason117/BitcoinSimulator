defmodule BitcoinSimulator.Simulation.Node do
  ...
  def handle_cast({:transaction, transaction, sender}, state) do
    {_new_tx, new_state} = handle_transaction_received(transaction, sender, state)
    {:noreply, new_state}
  end

  def handle_cast({:transaction_assembled, transaction, new_wallet}, state) do
    {_new_tx, new_state} = handle_transaction_assembled(transaction, new_wallet, state)
    {:noreply, new_state}
  end

  def handle_cast({:block, block, sender}, state) do
    {_new_block, new_state} = handle_block_received(block, sender, state)
    {:noreply, new_state}
  end

  def handle_cast({:block_mined, block, coinbase_addr}, state) do
    {_new_block, new_state} = handle_block_mined(block, coinbase_addr, state)
    {:noreply, new_state}
  end
